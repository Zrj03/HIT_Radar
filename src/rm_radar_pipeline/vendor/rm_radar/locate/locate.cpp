...
void Locator::update(
    ...
    std::for_each(
        std::execution::seq, depth_images_.begin(), depth_images_.end(),
...
void Locator::search(Robot& robot) const noexcept {
    ...
    for (int v = rect.y; v < rect.y + rect.height; ++v) {
        ...
        for (int u = rect.x; u < rect.x + rect.width; ++u) {
            ...
            auto point_it = point_index_map_.find(cv::Point2i(u, v));
            if (point_it == point_index_map_.end()) {
                continue;
            }
            int index = point_it->second;
            auto cluster_it = index_cluster_map_.find(index);
            int cluster_id = cluster_it != index_cluster_map_.end() ? cluster_it->second : -1;
