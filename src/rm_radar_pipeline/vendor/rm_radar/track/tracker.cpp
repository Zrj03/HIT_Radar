...
Tracker::Tracker(const cv::Point3f& observation_noise, int class_num,
                 ...
                 float hard_match_distance, float static_smooth_max_speed,
                 float static_smooth_radius, float static_smooth_alpha)
    : class_num_{class_num},
      ...
      hard_match_distance_{hard_match_distance},
      static_smooth_max_speed_{static_smooth_max_speed},
      static_smooth_radius_{static_smooth_radius},
      static_smooth_alpha_{std::clamp(static_smooth_alpha, 0.0f, 1.0f)} {}
...
void Tracker::update(
    ...
    for (size_t robot_id = 0; robot_id < match_result.size(); ++robot_id) {
        ...
        const float match_distance =
            calculateDistance(robot.location().value(), track.location());
        if (match_distance > hard_match_distance_ ||
            (match_distance > 2 * distance_thresh_ &&
             robot.label().value_or(-1) != track.label())) {
            unmatched_robot_indices.emplace_back(robot_id);
            continue;
        }

        // Updates track. When a target is nearly static, small locator noise
        // should not pull the filtered annotation around every frame.
        cv::Point3f measurement = robot.location().value();
        const float track_speed =
            calculateDistance(track.velocity(), cv::Point3f(0.0f, 0.0f, 0.0f));
        if (track.isConfirmed() && track_speed < static_smooth_max_speed_ &&
            match_distance < static_smooth_radius_) {
            const auto predicted_location = track.location();
            measurement = predicted_location * (1.0f - static_smooth_alpha_) +
                          measurement * static_smooth_alpha_;
        }
        track.update(measurement, robot.feature(class_num_));
