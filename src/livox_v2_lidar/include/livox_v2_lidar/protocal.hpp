...
constexpr uint16_t PARAMETER_CONFIGURATION = 0x0100;

struct pointcloud_host_ipcfg {
    little_uint16_t key = 0x0006;
    little_uint16_t length = 8;
    little_uint8_t host_ip[4];
    little_uint16_t dst_port = 57000;
    little_uint16_t src_port = 57000;
};

struct work_tgt_mode {
    little_uint16_t key = 0x001a;
    little_uint16_t length = 1;
    little_uint8_t value = 0x01;
};

struct parameter_configuration_start {
    little_uint16_t key_num = 2;
    little_uint16_t rsvd = 0;
    pointcloud_host_ipcfg pointcloud_host;
    work_tgt_mode work_mode;
};
#pragma pack(pop)
