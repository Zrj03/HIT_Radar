...
void LidarPublisher::recv_spin()
{
    ...
    while (rclcpp::ok() && socket->is_open()) {
        ...
        if (need_start) {
            ...
            protocal::parameter_configuration_start hs;
            auto host_bytes = host_ip.to_bytes();
            std::copy(host_bytes.begin(), host_bytes.end(), hs.pointcloud_host.host_ip);
            hs.pointcloud_host.dst_port = local_port;
            hs.pointcloud_host.src_port = local_port;
            socket->send_to(
                boost::asio::buffer(buf,
                    protocal::write_frame_buffer(buf.data(), protocal::PARAMETER_CONFIGURATION,
...
LidarPublisher::LidarPublisher(const rclcpp::NodeOptions& options) : rclcpp::Node("lidar_v2", options)
{
    ...
    declare_parameter("host_ip", "192.168.1.50");
    ...
    host_ip = boost::asio::ip::make_address_v4(get_parameter("host_ip").as_string());
    dest_ip = boost::asio::ip::make_address_v4(get_parameter("dest_ip").as_string());

    RCLCPP_INFO(get_logger(), "Params: pc_batch_size: %lu, line_num: %d, host_ip: %s, dest_ip: %s",
        batch_dot_num, line_num, host_ip.to_string().c_str(), dest_ip.to_string().c_str());
