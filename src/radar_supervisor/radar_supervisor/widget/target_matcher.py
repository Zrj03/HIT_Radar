...
class TargetMatcher:
    def __init__(self, node: Node) -> None:
        self.node = node
        self.button_ = StatusButton("rm_radar_pipeline", lambda: asyncio.ensure_future(self.create_sub()))
    ...
    async def create_sub(self) -> None:
        ...
        self.match_result_sub = self.node.create_subscription(
            topic='rm_radar_pipeline/match_result',
        ...
        self.image_sub = self.node.create_subscription(
            topic='rm_radar_pipeline/visualization',
            msg_type=Image,
    ...
    def marker_callback(self, msg) -> None:
        ...
        if msg:
            ...
            self.status = False
            self.button_.set_text(f'Failed: rm_radar_pipeline/match_result 没有消息')
    ...
    def image_callback(self, msg) -> None:
        ...
        if msg:
            ...
            self.status = False
            self.button_.set_text(f'Failed: rm_radar_pipeline/visualization 没有消息')
    ...
    async def wait_for_result(self) -> None:
        ...
        if not (self.enter_match_result and self.enter_image):
            ...
            self.button_.set_text(f"Success: rm_radar_pipeline 匹配结果正常")
        self.set_status()
